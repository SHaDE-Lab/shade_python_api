...
def Solweig_2021a_calc(dsm, vegdsm, dem, res, trans, svf, svfN, svfW, svfE, svfS, svfveg, svfNveg, svfEveg, svfSveg,
    ...
    ###Nighttime Estimation (SHaDE LAB)
        Tmrt= np.full(dsm.shape,Ta)
        ...
        if alt > 0:  # # # # # # DAYTIME # # # # # #
            ...
            # Shadow  images
            if usevegdem == 1:
                vegsh, sh, vbshvegsh, wallsh, wallsun, wallshve, facesh, facesun = shadowingfunction_wallheight_23(dsm,
                                                                                                                   vegdem,
                                                                                                                   vegdem2,
                                                                                                                   azmt,
                                                                                                                   alt, res,
            ...
            # sh-(1-vegsh)*(1-psi)
                ...
                sh, wallsh, wallsun, facesh, facesun = shadowingfunction_wallheight_13(dsm, azmt, alt, res, walls,
                                                                                       dirwalls * np.pi / 180.)
                shadow = sh
        ...
        if cyl == 1 and ani == 1:  # Human body considered as a cylinder with Perez et al. (1993)
            Sstr = absK * ((KsideI + KsideD) * Fcyl + (Kdown + Kup) * Fup + (
                        Knorth + Keast + Ksouth + Kwest) * Fside) + absL * (
