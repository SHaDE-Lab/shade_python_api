...
def findwalls(dsm, walllimit, outfolder):
    ...
    vf = rio.open(out, 'w', driver='GTiff', height=a.shape[0], width=a.shape[1], count=1, crs=DSM.crs,
                  transform=DSM.transform, dtype=walls.dtype)
    vf.write(walls, 1)
    vf.close
...
def filter1Goodwin_as_aspect_v3(res, dsm, outfolder):
    ...
    vf = rio.open(out, 'w', driver='GTiff', height=a.shape[0], width=a.shape[1], count=1, crs=DSM.crs,
                  transform=DSM.transform, dtype=dirwalls.dtype)
